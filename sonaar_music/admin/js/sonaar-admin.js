...
(function ($) {
  ...
  $(document).on(
    "change",
    "#inspector-select-control-0, #inspector-toggle-control-0, #inspector-toggle-control-1, #inspector-toggle-control-2, #inspector-toggle-control-3, #inspector-toggle-control-4",
    function (e) {
      // For Gutenberg
      setTimeout(function () {
        IRON.players = [];
  ...
  $("#post_audiopreview_all").on("change", function () {
    if (post_audiopreview_all_ready) {
      ...
      var selectedValue = $(this).val();
      // Set the value of all other selectors to match the main selector's value
      $('select[name^="alb_tracklist["][name$="[post_audiopreview]"]')
        .val(selectedValue)
        .trigger("change");
    }
    post_audiopreview_all_ready = true;
  ...
  function init_adminSubMenu_separator() {
    ...
    // Add separators after specified menu items
    addSeparator(
      '#menu-posts-sr_playlist li a[href^="edit-tags.php?taxonomy=playlist-tag&post_type=sr_playlist"]'
    );
    addSeparator(
      '#menu-posts-album li a[href^="edit-tags.php?taxonomy=playlist-tag&post_type=album"]'
    );
    //addSeparator('#menu-posts-sr_playlist li a[href^="edit.php?post_type=sr_email_submission"]');
    addSeparator('#menu-posts-sr_playlist li a[href^="edit.php?post_type=usage-terms"]');
    addSeparator(
      '#menu-posts-sr_playlist li a[href^="edit.php?post_type=sr_playlist&page=srmp3_settings_shortcodebuilder"]'
    );
    addSeparator(
      '#menu-posts-album li a[href^="edit.php?post_type=album&page=srmp3_settings_shortcodebuilder"]'
    );
  }
  function init_shortcodeBuilder() {
    if (!$("#cmb2-metabox-srmp3_settings_shortcodebuilder").length) return;
    ...
    window.addEventListener("beforeunload", function (e) {
      if (!isSaving) {
        // Check if the saving process is not happening
        e.preventDefault();
        e.returnValue = ""; // Necessary for the prompt to trigger
      }
    ...
    $('.cmb2-wrap input[type="text"], .cmb2-wrap input[type="number"], .cmb2-wrap textarea').on(
      "keydown",
      function (e) {
        // Check if the pressed key is Enter (key code 13)
        if (e.which === 13) {
    ...
    function init_JSFields_Handles() {
      $("#source").each(function () {
        ...
        if (selectedValue === "from_cat" && !$("#category1").length) {
          //If Category is empty, display notification
          var text = sonaar_admin_ajax.translations.category_not_found;
          const $newTextElement = $('<p style="color:red;">' + text + "</p>");
          $(this).after($newTextElement);
        }
      ...
      $(".cmb2-id-player-meta-group .cmb-repeatable-grouping").each(function () {
        var group = $(this);
        var selectedOptionText = group
          .find(".cmb-type-select select:first")
          .find(":selected")
          .text()
          .trim();
        //add the second select option if it exists
        var selectedOptionText2 = group
          .find(".cmb-type-select select:last")
          .find(":selected")
          .text()
          .trim();
        if (selectedOptionText2) {
      ...
      $('[data-target-selector*="{#}"]').each(function () {
        ...
        // First, handle the special cases for switches where IDs end with 'inline1' or 'inline2'
        if (/inline[12]$/.test(id)) {
          //WIP inline is the ID of he CMB2 switch
          // Extract the group number from the name attribute
          var nameAttr = input.attr("name");
          var groupNumberMatch = nameAttr.match(/\[(\d+)\]/); // Matches the first digit inside brackets
          if (groupNumberMatch && groupNumberMatch[1]) {
    ...
    // Attach event delegation to the parent container
    $("#cmb2-metabox-srmp3_settings_shortcodebuilder").on(
      "change",
    ...
    $(".cmb-repeatable-group").on(
      "cmb2_shift_rows_complete cmb2_remove_row cmb2_add_row",
    ...
    // Your existing DOM manipulation code
    var cmbRows = $(
      "#cmb2-metabox-srmp3_settings_shortcodebuilder .srmp3-player-preview-container"
    );
    var navTabWrapper = $("#shortcode_builder");
    if (navTabWrapper.length > 0) {
      navTabWrapper.append(cmbRows);
    ...
    var ps = new PerfectScrollbar(
      document.querySelector("#srmp3_settings_shortcodebuilder .cmb2-wrap.form-table"),
      {
        wheelSpeed: 1.25,
        swipeEasing: false,
        wheelPropagation: false,
  ...
  function init_shortcodeBuilder_buttons() {
    $("#shortcode-delete-template-bt").on("click", function (e) {
      ...
      var userConfirmation = confirm(
        'Are you sure you want to delete "' + $("#save_template_name").val() + '" template?'
      );
      if (!userConfirmation) {
        return;
      }
    ...
    $("#shortcode-export-template-bt").on("click", function (e) {
      ...
      $.post(sonaar_admin_ajax.ajax.ajax_url, data, function (response) {
        if (response.success) {
          ...
          $("#shortcode-export-template-bt .export-text").text("Exported to clipboard");
          //remove class dashicons-upload and add dashicons-yes
          $("#shortcode-export-template-bt .dashicons")
            .removeClass("dashicons-upload")
            .addClass("dashicons-yes");
          //change button background color to green
          $("#shortcode-export-template-bt").css("background-color", "#30c5c8");

          navigator.clipboard
            .writeText(JSON.stringify(response.data.template_values))
            .then(() => {
              // Temporarily change button text in the span
              alert("Template copied to clipboard! You can paste it in your import template box.");
            })
  ...
  function setBuilderBackgroundColor() {
    var bgColor = $(".cmb2-id-preview-bg-color .wp-color-result").css("background-color");
    document
      .getElementById("shortcode_builder")
      .style.setProperty("background-color", bgColor, "important");
    if (bgColor !== "rgb(246, 247, 247)") {
      // This is a user input color and not the default one. Do the magic
      document
        .getElementById("shortcode_builder")
        .style.setProperty("background-image", "unset", "important");
    } else {
      document.getElementById("shortcode_builder").style.removeProperty("background-image");
    }
  }
  function init_shortcodeBuilder_colorPicker() {
    ...
    const observer = new MutationObserver(mutations => {
      mutations.forEach(mutation => {
        if (mutation.type === "childList") {
          $(mutation.addedNodes)
            .find(".wp-color-result, .color-alpha")
            .each(function () {
              styleObserver.observe(this, { attributes: true, attributeFilter: ["style"] });
              ...
              if (!$(this).find(".srp-colopicker-inner-wrapper").length) {
                //so we can apply a checkergrid withut affecting the select color button
                $(this)
                  .find("span.wp-color-result-text")
                  .wrap('<span class="srp-colopicker-inner-wrapper"></span>');
              }
            ...
            if ($(node).is(".wp-color-result") || $(node).find(".wp-color-result").length > 0) {
              $(node)
                ...
                .each(function () {
                  ...
                  if (
                    $(node).find("input.wp-color-picker").val() === "" ||
                    $(node).find("input.wp-color-picker").val() === "#"
                  ) {
                    $(this).css("opacity", "0.8");
                    $(this).addClass("srp-colopicker-no-background-color");
  ...
  function sendAjaxRequest_for_builder() {
    ...
    $(
      "#cmb2-metabox-srmp3_settings_shortcodebuilder input, #cmb2-metabox-srmp3_settings_shortcodebuilder select, #cmb2-metabox-srmp3_settings_shortcodebuilder textarea"
    ).each(function () {
      var $this = $(this);
      var nameAttr = $this.attr("name");
      if (!nameAttr) return; // Skip if no name attribute
      ...
      if ($this.is(":checkbox")) {
        if ($this.closest(".cmb2-id-category").length) {
          // Replace with the specific container/class identifying the old fields
          data[name] = data[name] || { values: [], selectors: [] };
          if ($this.is(":checked")) {
            data[name].values.push($this.val());
            if (targetSelector) {
  ...
  function setShortcodeColors() {
    ...
    // Regular expression to match the specific shortcode opening and closing tags
    var regex =
      /(\[sonaar_audioplayer)|(\]\[\/sonaar_audioplayer\])|(\[|\/|\]|"|=)|(\w+)=["']([^"']+)["']/g;

    // Replace matched text with HTML span tags
    var highlightedShortcode = shortcode.replace(
      regex,
      function (match, open, close, special, arg, value) {
        if (open) {
          // Highlight the opening tag without the closing bracket
          return '<span class="shortcode-name">' + match + "</span>";
        } else if (close) {
          ...
          return (
            '<span class="argument">' +
            arg +
            '</span><span class="special">=</span><span class="special">"</span><span class="value">' +
            value +
    ...
    // Update the content of the <pre> tag with the highlighted shortcode
    // Sanitize: only allow span tags with specific classes (shortcode-name, special, argument, value)
    var sanitizedShortcode = highlightedShortcode.replace(
      /<(?!\/?span\s*(?:class="(?:shortcode-name|special|argument|value)")?\s*>)[^>]*>/gi,
      ""
    );
    preTag.innerHTML = sanitizedShortcode;
  }
  ...
  function init_lightDarkmodeSwitch() {
    ...
    function createToggle(dashiconClass, positionClass) {
      ...
      let iconElem = document.createElement("span");
      iconElem.className =
        "dashicons " + dashiconClass + " srmp3darkmode-toggle-icon " + positionClass;
      ...
      toggle.addEventListener("click", () => {
        ...
        if (isDarkMode) {
          iconElem.className =
            "fas fa-moon srmp3darkmode-toggle-icon srmp3darkmode-toggle-icon-right"; // Sun icon
          knob.style.transform = "translateX(0)";
          document.body.classList.remove("dark-mode");
        } else {
          iconElem.className =
            "fas fa-sun srmp3darkmode-toggle-icon srmp3darkmode-toggle-icon-left"; // Moon icon
          knob.style.transform = "translateX(20px)";
          document.body.classList.add("dark-mode");
        }
    ...
    if (document.body.classList.contains("dark-mode")) {
      darkModeToggle.firstChild.className =
        "fas fa-sun srmp3darkmode-toggle-icon srmp3darkmode-toggle-icon-left";
      darkModeToggle.lastChild.style.transform = "translateX(20px)";
    } else {
      darkModeToggle.firstChild.className =
        "fas fa-moon srmp3darkmode-toggle-icon srmp3darkmode-toggle-icon-right";
      darkModeToggle.lastChild.style.transform = "translateX(0)";
    }
  ...
  function init_srmp3_fixed_ui() {
    ...
    // Create version elements safely using DOM methods to prevent XSS
    var freeVerText = document.createTextNode("MP3 Audio Player Free ");
    var freeVerSpan = document.createElement("span");
    freeVerSpan.className = "srmp3-settings-pro-not-available";
    freeVerSpan.textContent =
      sonaar_music && sonaar_music.plugin_version_free
        ? sonaar_music.plugin_version_free
        : "Not available";

    var proVerText = document.createTextNode("MP3 Audio Player Pro ");
    var proVerSpan = document.createElement("span");
    proVerSpan.className = "srmp3-settings-pro-not-available";
    proVerSpan.textContent =
      sonaar_music && sonaar_music.plugin_version_pro
        ? sonaar_music.plugin_version_pro
        : "Not available";

    $(".nav-tab-wrapper").append(versionWrapper);

    // Append elements safely using DOM manipulation instead of innerHTML
    versionWrapper.appendChild(freeVerText);
    versionWrapper.appendChild(freeVerSpan);
    versionWrapper.appendChild(document.createElement("br"));
    versionWrapper.appendChild(proVerText);
    versionWrapper.appendChild(proVerSpan);
  ...
  function init_srmp3_importTemplates() {
    ...
    var action = $("body[class*='srmp3-import-shortcode-templates']").length
      ? "import_srmp3_shortcode_template"
      : "import_srmp3_elementor_template";
    srpTemplatesSearch = new List("srp_templates_container", options);
    ...
    $(".srmp3_import_overlay").click(function (e) {
      ...
      $.post(sonaar_admin_ajax.ajax.ajax_url, data, function (response) {
        var obj;
        if (action === "import_srmp3_shortcode_template") {
          // we dont deal with the same kind of response
          obj = response;
          if (obj.success === true) {
            //add the template name and replace space with - at the end of the href attr
            var template_name = obj.data.template_name;
            $(".srmp3_import_success a").attr(
              "href",
              $(".srmp3_import_success a").attr("href") + "&tmpl=" + template_name
            );
            $(".srmp3_import_success .srmp3-template-name").text(
        ...
        if (obj.success === true) {
          ...
          $(".srmp3_import_error_message").remove();
          $(".srmp3_import_failed").append(
            '<div class="srmp3_import_error_message">' + obj.message + "</div>"
          );
          $(".srmp3_import_failed").show();
        }
      });
  ...
  function init_srmp3_generate_bt() {
    ...
    if ($(".srmp3-generate-bt").length) {
      ...
      if (posts_in) {
        ...
        $("#audiopreview-settings-title").after(
          '<div style="width: fit-content;" class="notice notice-warning is-dismissible audiopreview_posts_in_notice">' +
            ...
            posts_in_count +
            " posts are ready to have their audio previews generated." +
            "</h2>" +
            "<p>Review the settings below and click <strong>Generate</strong> Button.</p>" +
            '<p style="font-size:10px;">Posts: ' +
            posts_in +
            "</p>" +
            "</div>"
        );
      ...
      setTimeout(function () {
        ...
        $(document).on(
          "click",
          ".file-status, .cmb2-upload-button, .cmb2-remove-file-button",
          handleInputChange
        );
      }, 2000);
      function handleInputChange(event) {
        ...
        // Check if the message is already present; if not, add it
        if ($("#saveChangeMessage").length === 0) {
          $("#srmp3_indexTracks").after(
            '<span id="saveChangeMessage" style="margin-left:10px; color:red;">Save changes and refresh this page before rebuiling index.</span>'
          );
          $(".srmp3-audiopreview-bt").after(
            '<span id="saveChangeMessage" style="margin-left:10px; color:red;">Save changes and refresh this page before generate previews.</span>'
          );
        }
      }
    }
  ...
  function init_srmp3_tools() {
    ...
    if (!isFirefox()) {
      $("#srmp3-settings-generatepeaks-bt-container").after(
        '<div style="margin-top:10px;color:red;"><strong>Warning:</strong> Peaks Generation is resource-intensive. For audio tracks longer than 30 minutes, consider using Firefox for better memory management. If your browser crashes during peak generation, try Firefox.</div><br>'
      );
    }

    $("#srmp3_convertcpt").click(function (e) {
      var userConfirmation = confirm(
        "Are you sure you want to export your old stuff into MP3 Audio Player? Please confirm"
      );
      if (userConfirmation) {
        continueIndexing = true;
        e.preventDefault();
        ...
        function srmp3_convertCPT(index, originalText, $clickedButton) {
          ...
          $.ajax({
            ...
            success: function (response) {
              ...
              if (response.totalPosts && response.processedPosts) {
                $clickedButton
                  .siblings("#progressText")
                  .text(response.processedPosts + " / " + response.totalPosts + " posts");
              }

              if (response.progress) {
                $clickedButton
                  .siblings("#indexationProgress")
                  .val(Math.round(Number(response.progress)));
              }
              ...
              if (response.completed) {
                ...
                //$clickedButton.siblings('#stopGeneratePeaksButton').css('display', 'none');
                $clickedButton
                  .siblings("#srmp3_indexTracks_status")
    ...
    $(".srmp3-generatepeaks-bt").click(function (e) {
      ...
      if (!$(this).siblings("#stopGeneratePeaksButton").length) {
        $(this).after(
          '<button id="stopGeneratePeaksButton" class="srmp3-stopgenerate-bt">Stop</button>'
        );
      }
      $(this).siblings("#srmp3_indexTracks_status").text("Processing...");
      ...
      function get_audio_files(index, originalText, $clickedButton, audioContext) {
        ...
        var overwrite = document.querySelector(".cmb2-id-peaks-overwrite .cmb2-enable.selected")
          ? "true"
          : "false";
        ...
        success: function (response) {
          ...
          var userConfirmation = confirm(
            "Time to cleanup !?\n\nAre you sure you want to remove the " +
              fileCount +
              " peak files on your server? Do not worry, this will NOT delete or remove any of your audio files."
          );
          if (userConfirmation) {
            // Start the removal process
            $.ajax({
  ...
  function init_srmp3_audioPreview() {
    ...
    if ($(".option-srmp3_settings_audiopreview .audiopreview-denied").length) {
      $(".option-srmp3_settings_audiopreview .cmb-row:not(:first-child):not(:nth-child(2))")
        .css("opacity", "0.5")
        .css("pointer-events", "none");
      $(".option-srmp3_settings_audiopreview .submit")
        .css("opacity", "0.5")
        .css("pointer-events", "none");
    }
    ...
    $(document).on("click", ".srmp3-audiopreview-bt", function (e) {
      ...
      if (postID) {
        //console.log('we are in a POST !!');
        var selectElem = parentRow
          .prevAll(".cmb-row")
          .find('select.cmb2_select[name^="alb_tracklist["]')
          .first();
        ...
        if (isGutenbergActive()) {
          ...
          if (hasCMB2Changes) {
            ...
            if (publishButton) {
              if (
                confirm(
                  "We must save the post first. Please click the Generate Preview again once the save is complete."
                )
              ) {
                publishButton.click();
              } else {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          var userConfirmation = confirm(
            "Time to cleanup !?\n\nAre you sure you want to remove the " +
              fileCount +
              " audio preview files on your server? You will need to re-generate them again."
          );
          if (userConfirmation) {
            // Start the removal process
            $.ajax({
    ...
    function indexAudioPreview_AJAX(offset, originalText, postID, index, clickedButton, posts_in) {
      ...
      //* start of Post EDIT */
      let customData = {};
      let audiourl = clickedButton
        .closest(".cmb-repeatable-grouping")
        .find(".srmp3-admin-track-player-container audio source")
        .attr("src");
      let audioTitle = clickedButton
        .closest(".cmb-repeatable-grouping")
        .find("#alb_tracklist_" + index + "_stream_title")
        .val();

      var $clickedButton = clickedButton.closest(".cmb-repeatable-grouping");
      var $targetElement = $clickedButton
        .find("#alb_tracklist_" + index + "_post_trimstart")
        .closest(".cmb-row");

      if ($targetElement.length && $targetElement.css("display") !== "none") {
        customData = {
          trimstart: clickedButton
            .closest(".cmb-repeatable-grouping")
            .find("#alb_tracklist_" + index + "_post_trimstart")
            .val(),
          previewLength: clickedButton
            .closest(".cmb-repeatable-grouping")
            .find("#alb_tracklist_" + index + "_post_audiopreview_duration")
            .val(),
          fadein: clickedButton
            .closest(".cmb-repeatable-grouping")
            .find("#alb_tracklist_" + index + "_post_fadein_duration")
            .val(),
          fadeout: clickedButton
            .closest(".cmb-repeatable-grouping")
            .find("#alb_tracklist_" + index + "_post_fadeout_duration")
            .val(),
          watermarkFile: clickedButton
            .closest(".cmb-repeatable-grouping")
            .find("#alb_tracklist_" + index + "_post_audio_watermark")
            .val(),
          watermarkGap: clickedButton
            .closest(".cmb-repeatable-grouping")
            .find("#alb_tracklist_" + index + "_post_audio_watermark_gap")
            .val(),
          prerollFile: clickedButton
            .closest(".cmb-repeatable-grouping")
            .find("#alb_tracklist_" + index + "_post_ad_preroll")
            .val(),
          postrollFile: clickedButton
            .closest(".cmb-repeatable-grouping")
            .find("#alb_tracklist_" + index + "_post_ad_postroll")
            .val(),
        };
      }
      //* end of Post EDIT */
        ...
        success: function (response) {
          ...
          if (response.message) {
            if (indexPos != trackLength && !completed) {
              $(".srmp3-post-all-audiopreview-bt")
                .siblings("#srmp3_indexTracks_status")
                .text(indexPos + " / " + trackLength + " " + response.message);
              clickedButton.siblings("#srmp3_indexTracks_status").text(response.message);
            }
          }
          if (response.completed) {
            ...
            if (!response.error) {
                ...
                $(".srmp3-post-all-audiopreview-bt")
                  ...
                  .html(
                    "Showtime! 🎉 ( " +
                      ...
                      " / " +
                      indexPos +
                      " ) <em>Don't forget to <strong>save</strong> this post.</em>"
                  );
                stopTimer($(".srmp3-post-all-audiopreview-bt"));
              }
              ...
              if (response.file_output != null && response.file_output != "") {
                ...
                clickableLink =
                  ...
                  "?_=" +
                  timestamp +
                  '" target="_blank">Listen Preview</a>';
                clickedButton.siblings("#progressText").text("");
                clickedButton
                  .siblings("#srmp3_indexTracks_status")
                  .html(
            ...
            indexAudioPreview_AJAX(
              offset + parseInt(sonaar_music_pro.option.preview_batch_size),
              originalText,
              postID,
              index,
              clickedButton,
        ...
        error: function (jqXHR, textStatus, errorThrown) {
          ...
          if (jqXHR.status == 500) {
            ...
            clickedButton
              .siblings("#srmp3_indexTracks_status")
              .text(
                "An error occurred. It might be due to the server max execution time limit reached."
              );
          } else {
            // Some other error
            clickedButton
  ...
  function init_toggleTracklistBox() {
    ...
    function toggleClosedClass() {
      ...
      sonaar_music.option.collapse_tracklist_backend =
        sonaar_music.option.collapse_tracklist_backend === "true" ? "false" : "true";
    }
  }
  ...
  function init_TrackTitleOnRepeater(newRow) {
    ...
    var inputFields = document.querySelectorAll(
      ".srmp3-cmb2-file input,  .sr-stream-url-field input, .srmp3-fileorstream select"
    );
    inputFields.forEach(function (inputField) {
      inputField.addEventListener("input", function () {
        clearTimeout(timeoutId);
    ...
    function onMutation(mutationsList, observer) {
      ...
      if (hasChildListMutation) {
        ...
        var isInsidePlayerContainer = mutationsList.some(mutation => {
          var target = mutation.target;
          while (target !== null) {
            if (target.classList.contains("srmp3-admin-track-player-container")) {
              // we dont want to create an infinite loop
              return true;
            }
            target = target.parentElement;
          }
  ...
  function addTrackTitletoTrackRepeater(el = null) {
    ...
    if (el) {
      ...
      var trackElements = document.querySelectorAll(
        "#alb_tracklist_repeat .cmb-repeatable-grouping"
      );
    }
    if (!addNewTrackButtonLabel) {
      addNewTrackButtonLabel = trackElements[0].querySelector(".cmb2-upload-button").value;
    ...
    trackElements.forEach(function (trackElement) {
      ...
      switch (selectedOptionValue) {
        ...
        case "stream":
          let streamElement = trackElement.querySelector(
            '.srmp3-cmb2-file input[name$="[stream_title]"]'
          );
          audioURL = trackElement.querySelector(
            '.sr-stream-url-field input[name$="[stream_link]"]'
          );
          audioURL = audioURL ? audioURL.value : "";
          var targetLocation = trackElement.querySelector(".cmb2-text-url").parentNode;
          $track = streamElement ? streamElement.value : "";
        ...
        case "icecast":
          let icecastElement = trackElement.querySelector(
            '.srmp3-cmb2-file input[name$="[icecast_link]"]'
          );
          audioURL = icecastElement ? icecastElement.value : "";
          var targetLocation = icecastElement ? icecastElement.closest(".cmb-td") : null;
          $track = icecastElement ? icecastElement.value : "";
      ...
      if (audioURL) {
        ...
        audioElement = trackElement.querySelector(".srmp3-admin-track-player-container audio");
        var audioSourceElement = trackElement.querySelector(
          ".srmp3-admin-track-player-container audio source"
        );
        if (audioSourceElement) {
          audioSourceElement.setAttribute("src", audioURL);
        }
