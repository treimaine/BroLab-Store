...
export const useTranslation = () => {
  ...
  // Default to English; honor explicit user preference from localStorage if present.
  useEffect(() => {
    try {
      const stored = typeof window !== "undefined" ? localStorage.getItem("brolab_language") : null;
      if (stored && translations[stored]) {
        setCurrentLanguage(stored);
        return;
      }
    } catch {}
    // Keep English as native default; do not auto-switch to geolocation language
