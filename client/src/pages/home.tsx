...
import { Play, Pause, Star, TrendingUp, Music, Info, Eye } from 'lucide-react';
...
import { FeaturedBeatsSkeleton, CarouselBeatSkeleton } from '@/components/FeaturedBeatsSkeleton';
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="text-center">
            ...
            <p className="text-xl lg:text-2xl text-gray-300 mb-10 max-w-3xl mx-auto leading-relaxed">
              Premium beats crafted by top producers. License instantly and take your music to the next level.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link href="/about">
                <Button variant="outline" className="text-lg px-8 py-4 border-2 border-[var(--accent-purple)] text-[var(--accent-purple)] hover:bg-[var(--accent-purple)] hover:text-white">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {isLoading ? (
              ...
              featuredBeats.map((beat: any) => (
                <Card key={beat.id} className="bg-[var(--dark-gray)] border-[var(--medium-gray)] overflow-hidden hover:border-[var(--accent-purple)] transition-all duration-300 group">
                  <CardContent className="p-0">
                    <div className="relative">
                      ...
                      <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <HoverPlayButton 
                          audioUrl={beat.meta_data?.find((meta: any) => meta.key === 'audio_url')?.value || '/api/placeholder/audio.mp3'}
                          size="lg"
                        />
                      </div>
                      <Badge className="absolute top-3 left-3 bg-[var(--accent-purple)]">Featured</Badge>
                    </div>
                    <div className="p-6">
                      <h3 className="text-xl font-bold text-white mb-2">{beat.name}</h3>
                      <p className="text-gray-400 mb-4">{beat.categories?.[0]?.name || 'Hip Hop'}</p>
                      <div className="flex items-center justify-between">
                        <span className="text-2xl font-bold text-[var(--accent-purple)]">
                          ${beat.price || '29.99'}
                        </span>
                        <Link href={`/product/${beat.id}`}>
                          <Button variant="outline" size="sm" className="border-[var(--accent-purple)] text-[var(--accent-purple)] hover:bg-[var(--accent-purple)] hover:text-white">
                            View Details
                          </Button>
                        </Link>
                      </div>
      ...
      <section className="py-20 bg-[var(--deep-black)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-16">
            ...
            <Link href="/shop">
              <Button variant="outline" className="border-[var(--accent-purple)] text-[var(--accent-purple)] hover:bg-[var(--accent-purple)] hover:text-white">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading ? (
              ...
              trendingBeats.map((beat: any, index: number) => (
                <Card key={beat.id} className="bg-[var(--dark-gray)] border-[var(--medium-gray)] hover:border-[var(--accent-purple)] transition-all duration-300">
                  <CardContent className="p-4">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-400">{beat.categories?.[0]?.name || 'Hip Hop'}</p>
                        <div className="flex items-center justify-between mt-2">
                          <span className="text-[var(--accent-purple)] font-bold">${beat.price || '29.99'}</span>
      ...
      <section className="py-20 bg-gradient-to-r from-[var(--accent-purple)] to-[var(--accent-cyan)]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-white mb-8 opacity-90">
            Join thousands of artists who trust BroLab Entertainment for their music production needs.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/shop">
            ...
            <Link href="/membership">
              <Button variant="outline" className="border-2 border-white text-white hover:bg-white hover:text-[var(--accent-purple)] font-bold py-3 px-8 rounded-lg transition-colors">Subscribe</Button>
            </Link>
          </div>
        </div>
      </section>
