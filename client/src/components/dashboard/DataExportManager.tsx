...
const getTypeIcon = (type: ExportType): React.JSX.Element => {
  switch (type) {
    case "csv":
...
export const DataExportManager: React.FC<DataExportManagerProps> = ({
  ...
  const handleTemplateExport = useCallback(
    async (template: ExportTemplate): Promise<void> => {
      setIsExporting(true);
      try {
        await onExport(template);
        toast.success(`Export "${template.name}" started successfully`);
      } catch (err: unknown) {
        console.error("Export error:", err);
        toast.error("Error starting export");
      } finally {
  ...
  const handleCustomExport = useCallback(async (): Promise<void> => {
    ...
    >
      ...
      <div className="flex space-x-1 bg-gray-900/50 p-1 rounded-lg border border-gray-700/50">
        {[
          ...
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id as "templates" | "custom" | "history")}
        ...
        {activeTab === "templates" && (
          ...
          >
            {DEFAULT_TEMPLATES.map(template => (
              <Card
                key={template.id}
                className="bg-gray-900/50 border-gray-700/50 backdrop-blur-sm hover:border-purple-500/50 transition-all"
              >
                <CardHeader>
                  <div className="flex items-center justify-between">
          ...
          >
            <Card className="bg-gray-900/50 border-gray-700/50 backdrop-blur-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Créez un export sur mesure avec les champs de votre choix
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <div>
                      <Label htmlFor="export-name" className="text-white">
                        Nom de l&apos;export
                      </Label>
                      <Input
                        id="export-name"
                    ...
                    <div>
                      <Label htmlFor="export-description" className="text-white">
                      ...
                      <Input
                        ...
                        onChange={e =>
                          setCustomExport(prev => ({ ...prev, description: e.target.value }))
                        }
