...
import { Check, Clock, Download, FileAudio, Folder } from "lucide-react";
...
export function BeatStemsDelivery({
  ...
  onDownload,
  onDownloadAll,
}: Readonly<BeatStemsDeliveryProps>): React.ReactElement {
  const [downloadProgress] = useState<{ [key: string]: number }>({});
  ...
  // Helper function to get license badge color
  const getLicenseBadgeColor = (license: string): string => {
    if (license === "unlimited") return "bg-[var(--accent-purple)]";
    if (license === "premium") return "bg-[var(--accent-cyan)]";
    return "bg-gray-600";
  };
      ...
      <Card className="card-dark">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge className={`${getLicenseBadgeColor(licenseType)} text-white`}>
              {licenseType.charAt(0).toUpperCase() + licenseType.slice(1)} License
            </Badge>
          </div>
          <p className="text-gray-400">{beatTitle} - Available stems for your license type</p>
        </CardHeader>
          ...
          <div className="grid grid-cols-3 gap-4 p-4 bg-[var(--medium-gray)] rounded-lg">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-400">
                {availableStems.reduce(
                  (total, stem) => total + Number.parseInt(stem.size.replaceAll(/[^\d]/g, ""), 10),
                  0
                )}
                MB
              </div>
            ...
            {availableStems.map(stem => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  {Boolean(downloadProgress[stem.id]) && (
                    <div className="w-24">
          ...
          {licenseType !== "unlimited" && (
            <Card className="bg-gradient-to-r from-[var(--accent-purple)]/10 to-[var(--accent-cyan)]/10 border-[var(--accent-purple)]/20">
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium text-white mb-1">Unlock More Stems</h4>
                  ...
                  <Button className="btn-primary">Upgrade License</Button>
                </div>
              </CardContent>
            </Card>
          )}
