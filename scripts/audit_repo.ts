...
// Read current MISSING_FEATURES.md if exists
let missingFeaturesContent = '';
try {
  missingFeaturesContent = readFileSync('MISSING_FEATURES.md', 'utf8');
} catch {
  ...
  advancedFeatures: {
    ...
    multiLanguage: glob.sync('**/*.ts?(x)').some(f => {
      try {
        ...
        return content.includes('i18n') || content.includes('translation') || content.includes('locale');
      } catch { return false; }
    })
  }
};
...
try {
  const healthCheck = run('curl -s http://localhost:5000/api/health || echo "Server not reachable"');
...
console.log(`📁 Files Scanned: ${Object.values(report.fileStructure).reduce((a, b: any) => a + Object.values(b).filter(v => typeof v === 'number').reduce((x: any, y: any) => x + y, 0), 0)} total`);
