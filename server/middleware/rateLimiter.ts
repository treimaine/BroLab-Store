...
export class RateLimiter {
  ...
  async middleware(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const key = this.config.keyGenerator ? this.config.keyGenerator(req) : `${userId}:${this.action}`;
      const now = new Date();
      ...
      res.end = function(chunk?: any, encoding?: any) {
        if (!requestCompleted) {
          ...
          if (shouldCount) {
            ...
            self.updateCounter(userId, recordId, currentCount + 1, windowStart).catch(console.error);
          }
        }
  ...
  private async updateCounter(userId: number, recordId: string | null, newCount: number, windowStart: Date) {
    try {
      if (recordId) {
