 ...
 */
export function generateBeatOpenGraph(
  beat: BeatProduct,
  ...
  return {
    ...
    image: (beat as any).image_url || config.defaultImage || `${baseUrl}/logo.png`,
    type: 'music.song',
    siteName,
    twitterCard: 'summary_large_image',
    twitterHandle: config.twitterHandle,
    audioUrl: (beat as any).audio_url,
    duration: beat.duration ? `${Math.floor(beat.duration / 60)}:${(beat.duration % 60).toString().padStart(2, '0')}` : undefined,
...
export function generateOpenGraphHTML(meta: OpenGraphMeta): string {
  ...
  if (meta.twitterHandle) {
    tags.push(`<meta name="twitter:site" content="${meta.twitterHandle}" />`);
    tags.push(`<meta name="twitter:creator" content="${meta.twitterHandle}" />`);
  }
  
  // Ajouter les meta tags spécifiques à la musique pour les beats
  if (meta.type === 'music.song') {
    if (meta.audioUrl) {
      tags.push(`<meta property="og:audio" content="${meta.audioUrl}" />`);
