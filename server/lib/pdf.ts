...
export function buildInvoicePdfStream(
  ...
  doc.fontSize(10).text(`Facture n° ${order.invoice_number || ''}`, { align: 'right', width: doc.page.width - 200 });
  doc.moveDown();
  doc.fillColor('#000');
  ...
  doc.fontSize(10).text('ID', 40, tableTop).text('Licence', 120, tableTop).text('Prix', 200, tableTop).text('Qté', 270, tableTop).text('Total', 320, tableTop);
  let y = tableTop + 15;
  items.forEach(item => {
    doc.text(String(item.beat_id ?? item.id ?? ''), 40, y)
