...
describe('Port CLI logic (prompt & flags)', () => {
  ...
  it('2) TTY + prompt "A" (auto) : promptForPort renvoie auto:true, findFreePort appelé', async () => {
    ...
    const promptMock = jest.spyOn(portUtils, 'promptForPort').mockResolvedValue({ port: 0, auto: true });
    const findFreePortMock = jest.spyOn(portUtils, 'findFreePort')
  ...
  it('3) TTY, user tape un port manuel', async () => {
    process.argv = ['node', 'server/index.ts'];
    (process.stdin as any).isTTY = true;
    const promptMock = jest.spyOn(portUtils, 'promptForPort').mockResolvedValue({ port: 5052, auto: false });
    const findFreePortMock = jest.spyOn(portUtils, 'findFreePort')
      .mockRejectedValueOnce(new Error('EADDRINUSE'))
  ...
  it('5) Erreur : aucun port libre', async () => {
    process.argv = ['node', 'server/index.ts', '--auto'];
    (process.stdin as any).isTTY = false;
    const findFreePortMock = jest.spyOn(portUtils, 'findFreePort').mockRejectedValue(new Error('Aucun port libre'));
    const opts: CliOptions = {
      base: 5000,
      maxTries: 2,
      auto: true,
