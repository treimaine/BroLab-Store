...
import { useDataValidation } from "../../client/src/hooks/useDataValidation";
import {
  DataFreshnessMonitor,
  destroyDataFreshnessMonitor,
  getDataFreshnessMonitor,
} from "../../client/src/services/DataFreshnessMonitor";
import {
  DataValidationService,
  destroyDataValidationService,
  getDataValidationService,
} from "../../client/src/services/DataValidationService";
import type { DashboardData } from "../../shared/types/dashboard";
import type { ConsistentUserStats } from "../../shared/types/sync";

// Mock the dashboard store
jest.mock("../../client/src/store/useDashboardStore", () => ({
...
jest.mock("../../client/src/services/ErrorLoggingService", () => ({
  getErrorLoggingService: jest.fn(() => ({
    logError: jest.fn(),
...
describe("DataValidationService", () => {
  ...
  describe("validateDataIntegrity", () => {
    ...
    it("should detect mock data indicators", async () => {
      ...
      expect(
        mockIndicators.some((indicator: { field: string }) => indicator.field.includes("email"))
      ).toBe(true);
      expect(
        mockIndicators.some((indicator: { type: string }) => indicator.type === "placeholder_text")
      ).toBe(true);
    });
    ...
    it("should detect data inconsistencies", async () => {
      ...
      expect(
        inconsistencies.some((inc: { sections: string[] }) => inc.sections.includes("favorites"))
      ).toBe(true);
      expect(inconsistencies.some((inc: { type: string }) => inc.type === "calculation")).toBe(
        true
      );
    });
    ...
    it("should validate data freshness", async () => {
      const staleData = createMockDashboardData({
        stats: {
          ...(createMockDashboardData().stats as ConsistentUserStats),
    ...
    it("should provide appropriate recommendations", async () => {
      ...
      expect(
        report.recommendations.some((rec: { type: string }) => rec.type === "refresh_data")
      ).toBe(true);
    });
  });
...
describe("DataFreshnessMonitor", () => {
  ...
  describe("checkFreshness", () => {
    ...
    it("should detect stale sections", async () => {
      ...
      const useDashboardStoreModule = await import("../../client/src/store/useDashboardStore");
      const { useDashboardStore } = useDashboardStoreModule;
...
describe("useDataValidation hook", () => {
  ...
  it("should handle validation errors gracefully", async () => {
    ...
    const mockValidationService = {
      validateDataIntegrity: jest.fn().mockRejectedValue(new Error("Validation failed")),
      getDataFreshnessIndicator: jest.fn(),
      shouldRefreshData: jest.fn(),
    };

    jest.doMock("@/services/DataValidationService", () => ({
